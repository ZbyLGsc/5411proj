...
// SEA
/* Credit: some of the following ocean code is borrowed from
 * https://www.shadertoy.com/view/Ms2SD1 posted by user 'TDM' */

#define SEA_HEIGHT \
  1.0  // this is how many units from the top of the ocean bounding box
#define SEA_FREQ \
  1.5  // wave density: lower = spread out, higher = close together
#define SEA_CHOPPY \
  2.0  // smaller beachfront-type waves, they travel in parallel
...
// CLOUDS
/* Credit: some of the following cloud code is borrowed from
 * https://www.shadertoy.com/view/XtBXDw posted by user 'valentingalea' */
...
float noise3D(in vec3 p) { return texture(t_PerlinNoise, p.xz).x; }

const mat3 m =
    1.21 * mat3(0.00, 0.80, 0.60, -0.80, 0.36, -0.48, -0.60, -0.48, 0.64);
...
//-----------------------------------------------------------------------------------------------------------------------------------------------------------------------
{
  ...
  // 	{
  // 		t = d;
  // 		intersec.normal = normalize( cross(quads[i].v1 - quads[i].v0,
  // quads[i].v2 - quads[i].v0) ); 		intersec.emission =
  // quads[i].emission; 		intersec.color = quads[i].color; 		intersec.type =
  // quads[i].type;
  // 	}
  //     }

  for (int i = 0; i < N_OPENCYLINDERS; i++) {
    d = OpenCylinderIntersect(openCylinders[i].pos1, openCylinders[i].pos2,
                              openCylinders[i].radius, r, normal);
    if (d < t) {
      t = d;
  ...
  for (int i = 0; i < N_SPHERES; i++) {
    ...
    if (d < t) {
      t = d;
      intersec.normal =
          normalize((r.origin + r.direction * t) - spheres[i].position);
  ...
  if (d > 4000.0) {
    ...
    d = DisplacementBoxIntersect(vec3(-INFINITY, -INFINITY, -INFINITY),
                                 vec3(INFINITY, waterWaveHeight, INFINITY), r);
    hitWorldSpace = r.origin + r.direction * d;
  }
  ...
  if (d < t) {
    float eps = 1.0;
    t = d;
    float dx = getOceanWaterHeight_Detail(hitWorldSpace - vec3(eps, 0, 0)) -
               getOceanWaterHeight_Detail(hitWorldSpace + vec3(eps, 0, 0));
    float dy = eps * 2.0;  // (the water wave height is a function of x and z,
                           // not dependent on y)
    float dz = getOceanWaterHeight_Detail(hitWorldSpace - vec3(0, 0, eps)) -
               getOceanWaterHeight_Detail(hitWorldSpace + vec3(0, 0, eps));
...
vec3 CalculateRadiance(Ray r, vec3 sunDirection, inout uvec2 seed,
                       inout bool rayHitIsDynamic)
//----------------------------------------------------------------------------------------------
{
  spheres[0] = Sphere(90.0, vec3(500, uTestHeight, 25), vec3(0), vec3(0.8, 0.7, 0.4),
                      DIFF);  // White Ball

  Intersection intersec;
  Ray firstRay;

  vec3 randVec = vec3(rand(seed) * 2.0 - 1.0, rand(seed) * 2.0 - 1.0,
                      rand(seed) * 2.0 - 1.0);
  Ray cameraRay = r;
  vec3 initialSkyColor = Get_Sky_Color(r, sunDirection);

  Ray skyRay =
      Ray(r.origin * vec3(0.02),
          normalize(vec3(r.direction.x, abs(r.direction.y), r.direction.z)));
  float dc = SphereIntersect(
      20000.0,
      vec3(skyRay.origin.x, -19900.0, skyRay.origin.z) + vec3(rand(seed) * 2.0),
      skyRay);
  vec3 skyPos = skyRay.origin + skyRay.direction * dc;
  vec4 cld = render_clouds(skyRay, skyPos, sunDirection);

  Ray cloudShadowRay =
      Ray(r.origin * vec3(0.02), normalize(sunDirection + (randVec * 0.05)));
  float dcs = SphereIntersect(
      20000.0,
      vec3(skyRay.origin.x, -19900.0, skyRay.origin.z) + vec3(rand(seed) * 2.0),
      cloudShadowRay);
  vec3 cloudShadowPos = cloudShadowRay.origin + cloudShadowRay.direction * dcs;
  float cloudShadowFactor =
      checkCloudCover(cloudShadowRay.direction, cloudShadowPos);
  ...
  float weight;
  float t = INFINITY;
  float diffuseColorBleeding =
      0.5;  // range: 0.0 - 0.5, amount of color bleeding between surfaces
  ...
  for (int bounces = 0; bounces < 5; bounces++) {
    ...
    if (bounces == 0 && intersec.type == DIFF) rayHitIsDynamic = false;
    ...
    if (t == INFINITY) {
      ...
      if (bounces == 1 &&
          firstTypeWasSPEC)  // ray bounces off mirror and hits sky
      {
        skyHit = true;
        firstX = skyPos;
      ...
      if (firstTypeWasREFR) {
        if (!reflectionTime) {
          ...
          if (sampleLight || bounceIsSpecular) accumCol = mask * skyColor;

          // start back at the refractive surface, but this time follow
          // reflective branch
          r = firstRay;
        ...
        if (bounceIsSpecular || sampleLight)
          accumCol += mask * skyColor;  // add reflective result to the
                                        // refractive result (if any)
      ...
      if (firstTypeWasDIFF) {
        if (!shadowTime) {
          ...
          // start back at the diffuse surface, but this time follow shadow ray
          // branch
          r = firstRay;
          r.direction = normalize(r.direction);
          mask = firstMask;
        ...
        accumCol +=
            mask * skyColor *
            0.5;  // add shadow ray result to the colorbleed result (if any)
        break;
      }

      if (bounceIsSpecular || sampleLight)
        accumCol =
            mask * skyColor;  // looking at HDRI sky light through a reflection
    ...
    if (intersec.type == SEAFLOOR) {
      ...
      if (firstTypeWasREFR) {
        ...
        accumCol += mask * intersec.color * waterDotSun *
                    waterDotCamera;  // add reflective result to the refractive
                                     // result (if any)
        break;
      }
    ...
    // if we get here and sampleLight is still true, shadow ray failed to find a
    // light source
    if (sampleLight) {
      if (firstTypeWasREFR && !reflectionTime) {
        // start back at the refractive surface, but this time follow reflective
        // branch
        r = firstRay;
        r.direction = normalize(r.direction);
        mask = firstMask;
    ...
    if (intersec.type == DIFF)  // Ideal DIFFUSE reflection
    {
      ...
      if (diffuseCount == 1 && !firstTypeWasDIFF && !firstTypeWasREFR) {
        ...
        firstRay = Ray(
            x, normalize(
                   sunDirection));  // create shadow ray pointed towards light
        firstRay.direction = normalize(
            randomDirectionInSpecularLobe(firstRay.direction, 0.02, seed));
        firstRay.origin += nl * uEPS_intersect;

        weight = max(0.0, dot(firstRay.direction, nl)) *
                 0.005;  // down-weight directSunLight contribution
        ...
        continue;
      } else if (firstTypeWasREFR && diffuseCount == 1 &&
                 rand(seed) < diffuseColorBleeding) {
        // choose random Diffuse sample vector
        r = Ray(x, normalize(randomCosWeightedDirectionInHemisphere(nl, seed)));
        r.origin += nl * uEPS_intersect;
        continue;
      ...
      r = Ray(x, normalize(
                     sunDirection));  // create shadow ray pointed towards light
      r.direction =
          normalize(randomDirectionInSpecularLobe(r.direction, 0.02, seed));
      r.origin += nl * uEPS_intersect;
    ...
    if (intersec.type == REFR)  // Ideal dielectric REFRACTION
    {
      ...
      if (diffuseCount == 0 && !firstTypeWasREFR) {
        ...
        firstMask = mask * Re;
        firstRay = Ray(
            x, reflect(r.direction, nl));  // create reflection ray from surface
        firstRay.origin += nl * uEPS_intersect;
        mask *= Tr;
    ...
    if (intersec.type ==
        WOOD)  // Diffuse object underneath with thin layer of Water on top
    {
      checkOcean = false;
  ...
  if (skyHit)  // sky and clouds
  {
    ...
    vec3 sunColor = clamp(
        Get_Sky_Color(Ray(skyPos, normalize((randVec * 0.03) + sunDirection)),
                      sunDirection),
        0.0, 1.0);

    cloudColor *=
        mix(sunColor, vec3(1), max(0.0, dot(vec3(0, 1, 0), sunDirection)));
    cloudColor = mix(initialSkyColor, cloudColor, clamp(cld.a, 0.0, 1.0));

    hitDistance = distance(skyRay.origin, skyPos);
    accumCol = mask * mix(accumCol, cloudColor,
                          clamp(exp2(-hitDistance * 0.004), 0.0, 1.0));
  } else  // terrain and other objects
  {
    hitDistance = distance(cameraRay.origin, firstX);
    accumCol = mix(initialSkyColor, accumCol,
                   clamp(exp2(-log(hitDistance * 0.00003)), 0.0, 1.0));

    // underwater fog effect
    hitDistance = distance(cameraRay.origin, firstX);
    hitDistance *= uCameraUnderWater;
    accumCol = mix(vec3(0.0, 0.05, 0.05), accumCol,
                   clamp(exp2(-hitDistance * 0.001), 0.0, 1.0));
  }

  return max(vec3(0),
             accumCol);  // prevents black spot artifacts appearing in the water
}
...
//-----------------------------------------------------------------------
{
  ...
  // quads[0] = Quad( vec3(  0.0, 0.0,-559.2), vec3(549.6, 0.0,-559.2),
  // vec3(549.6, 548.8,-559.2), vec3(  0.0, 548.8,-559.2),    z, vec3(0.9),
  // DIFF);// Back Wall quads[1] = Quad( vec3(  0.0, 0.0,   0.0), vec3(  0.0,
  // 0.0,-559.2), vec3(  0.0, 548.8,-559.2), vec3(  0.0, 548.8,   0.0),    z,
  // vec3(0.7, 0.12,0.05),  DIFF);// Left Wall Red quads[2] = Quad( vec3(549.6,
  // 0.0,-559.2), vec3(549.6, 0.0,   0.0), vec3(549.6, 548.8,   0.0),
  // vec3(549.6, 548.8,-559.2),    z, vec3(0.2, 0.4, 0.36),  DIFF);// Right Wall
  // Green
  // //quads[3] = Quad( vec3(  0.0, 548.8,-559.2), vec3(549.6, 548.8,-559.2),
  // vec3(549.6, 548.8,   0.0), vec3(0.0, 548.8, 0.0),  z, vec3(0.9),  DIFF);//
  // Ceiling quads[3] = Quad( vec3(  0.0, 0.0,   0.0), vec3(549.6, 0.0,   0.0),
  // vec3(549.6, 0.0,-559.2), vec3(  0.0, 0.0,-559.2),    z, vec3(0.9), DIFF);//
  // Floor

  openCylinders[0] =
      OpenCylinder(50.0, vec3(50, 0, -50), vec3(50, -1000, -50), z,
                   vec3(0.05, 0.0, 0.0), WOOD);  // wooden support OpenCylinder
  openCylinders[1] =
      OpenCylinder(50.0, vec3(500, 0, -50), vec3(500, -1000, -50), z,
                   vec3(0.05, 0.0, 0.0), WOOD);  // wooden support OpenCylinder
  openCylinders[2] =
      OpenCylinder(50.0, vec3(50, 0, -510), vec3(50, -1000, -510), z,
                   vec3(0.05, 0.0, 0.0), WOOD);  // wooden support OpenCylinder
  openCylinders[3] =
      OpenCylinder(50.0, vec3(500, 0, -510), vec3(500, -1000, -510), z,
                   vec3(0.05, 0.0, 0.0), WOOD);  // wooden support OpenCylinder

  boxes[0] = Box(vec3(-82.0, -170.0, -80.0), vec3(82.0, 170.0, 80.0), z,
                 vec3(1.0), SPEC);  // Tall Mirror Box Left
  boxes[1] = Box(vec3(-86.0, -85.0, -80.0), vec3(86.0, 85.0, 80.0), z,
                 vec3(0.9), DIFF);  // Short Diffuse Box Right

  spheres[0] = Sphere(90.0, vec3(500, 90, 25), z, vec3(0.8, 0.7, 0.4),
                      DIFF);  // White Ball
  spheres[1] = Sphere(90.0, vec3(-500, 90, 0), z, vec3(0.9, 0.4, 0.0),
                      DIFF);  // Yellow Ball
  spheres[2] =
      Sphere(90.0, vec3(50, 90, 0), z, vec3(0.25, 0.0, 0.0), DIFF);  // Red Ball
}

// tentFilter from Peter Shirley's 'Realistic Ray Tracing (2nd Edition)' book,
// pg. 60
...
void main(void) {
  ...
  // vec3 camPos   = vec3( uCameraMatrix[3][0],  uCameraMatrix[3][1],
  // uCameraMatrix[3][2]);

  vec3 camRight =
      vec3(uCameraMatrix[0][0], uCameraMatrix[0][1], uCameraMatrix[0][2]);
  vec3 camUp =
      vec3(uCameraMatrix[1][0], uCameraMatrix[1][1], uCameraMatrix[1][2]);
  vec3 camForward =
      vec3(-uCameraMatrix[2][0], -uCameraMatrix[2][1], -uCameraMatrix[2][2]);
  ...
  // pixelOffset ranges from -1.0 to +1.0, so only need to divide by half
  // resolution
  pixelOffset /= (uResolution * 1.0);  // normally this is * 0.5, but for
                                       // dynamic scenes, * 1.0 looks sharper

  // we must map pixelPos into the range -1.0 to +1.0
  pixelPos = (gl_FragCoord.xy / uResolution) * 2.0 - 1.0;
  pixelPos += pixelOffset;

  vec3 rayDir = normalize(pixelPos.x * camRight * uULen +
                          pixelPos.y * camUp * uVLen + camForward);
  ...
  vec3 randomAperturePos =
      (cos(randomAngle) * camRight + sin(randomAngle) * camUp) *
      sqrt(randomRadius);
  ...
  vec3 pixelColor =
      CalculateRadiance(ray, uSunDirection, seed, rayHitIsDynamic);
